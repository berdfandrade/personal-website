      ...
      <Grid templateColumns="repeat(1, 1fr)">
        <Box>
          ...
          <Flex flexDirection={"row"}>
            <Heading fontSize={40}>{idade.anos} years</Heading>
            <Icon boxSize={10} mt={2} ml={5} as={IoMdTime}></Icon>
          </Flex>

          <Text fontSize={25}>{idade.meses} months</Text>
        </Box>
        <Box>
          <Text fontSize={20}>{Math.round(idade.dias)} days</Text>
          <Text fontSize={18}>
            {idade.horas} hours {idade.minutos} minutes
          </Text>

          <Text>{idade.segundos} seconds</Text>
          <Stat>
